...
function ServicesPanel() {
  ...
  const buildServicePayload = (values, allowClear) => {
    const payload = {};
    ...
    else if (allowClear && values.description !== undefined) payload.description = null;

    const price = (values.price || "").trim();
    if (price) payload.price = price;
    else if (allowClear && values.price !== undefined) payload.price = null;

    const duration = (values.duration || "").trim();
    if (duration) payload.duration = duration;
    else if (allowClear && values.duration !== undefined) payload.duration = null;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const payload = buildServicePayload(form, !!editing);
      if (!payload.name) {
      ...
      if (editing) {
        const data = await adminUpdateService(editing, payload);
...
function SpecialistsPanel() {
  ...
  const buildSpecialistPayload = (values, allowClear) => {
    const payload = {};
    const name = (values.name || "").trim();
    if (name) payload.name = name;
    ...
    else if (allowClear && values.experience !== undefined) payload.experience = null;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const payload = buildSpecialistPayload(form, !!editing);
      ...
      if (editing) {
        const data = await adminUpdateSpecialist(editing, payload);
        setRows((r) => r.map((sp) => (sp.specialistId === editing ? data.specialist : sp)));
      } else {
        const data = await adminCreateSpecialist(payload);
